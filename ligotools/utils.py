...
def reqshift(data: np.ndarray, fshift: float = 100.0, sample_rate: int = 4096) -> np.ndarray:
    ...
    ----------
    data : np.ndarray
        Real-valued time series (ideally already band-passed).
    fshift : float
        Frequency shift in Hz (positive shifts up).
    sample_rate : int
        Sample rate in Hz.
    ...
    -------
    np.ndarray
        Real-valued shifted signal (same length).
    """
    ...
    # Zero-wrap to reduce circular leakage
    if nbins > 0:
        Y[:nbins] = 0.0
    elif nbins < 0:
        Y[nbins:] = 0.0
...
def write_wavfile(path: Union[str, Path], fs: int, data: np.ndarray,
    ...
    ----------
    path : str | Path
        Output filename (e.g., Path('audio') / f'{eventname}_H1_whitenbp.wav').
    fs : int
        Sample rate (Hz).
    data : np.ndarray
        Audio samples (1D or slice).
    normalize : bool
        Scale to peak 1.0 before casting (default True).
    dtype : {'int16', 'float32'}
        Output sample type.
...
def plot_psd(strain: np.ndarray, fs: float, *,
             ...
             ax=None):
    """
    Compute and plot PSD (or ASD) using Welch's method with a window/overlap
    choice that minimizes spectral leakage—matching the intent of the notebook cell.
    ...
    label : str
    nperseg : int
        Defaults to 4*fs (like NFFT=4*fs in the notebook).
    noverlap : int
        Defaults to 50% overlap.
    window : str
        E.g., 'blackman' (notebook uses np.blackman(NFFT)).
    fmin, fmax : float
        Frequency limits for the plot (log-log).
    asd : bool
        If True, plot sqrt(PSD). If False, plot PSD.
    ...
    x = np.asarray(strain, dtype=float)
    if nperseg is None:
        nperseg = int(4 * fs)  # like NFFT = 4*fs in the notebook
    if noverlap is None:
        noverlap = nperseg // 2
